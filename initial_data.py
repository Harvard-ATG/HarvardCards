...
collection_json = [{"model":"flash.Collection", "pk":collection_pk, "fields":{"title":collection_name, "description":collection_description}}]
...
for i in range(0, decks):
    ...
    decks_json.append({"model":"flash.Deck", "pk":i+initial_deck_pk, "fields":{"title":deck_title, "collection" : collection_pk}})
...
for k in range(0, cards):
    ...
    cards_json.append({"model":"flash.Card", "pk":sort_order, "fields":{"sort_order": sort_order,"collection" : collection_pk}})
...
for k in range(0, cards):
    ...
    for j in range(0, num_fields):
        ...
        pk = pk+1
        fields_json.append({"model":"flash.Field", "pk":field_pk, "fields":{"sort_order": sort_order,"collection" : collection_pk, "label": label, "display": display, "show_label":show_label, "field_type":field_type}})
        cards_fields_json.append({"model":"flash.Cards_Fields", "pk":field_pk, "fields":{"sort_order": sort_order, "value": value, "field":pk, "card":card}})
...
for i in range(0, decks):
    ...
    for l in range(0, num_deck_cards):
        ...
        card_sort = l+1
        decks_cards_json.append({"model":"flash.Decks_cards", "pk":dc_pk, "fields":{"deck":deck_pk, "card" :card, "sort_order":card_sort}})
